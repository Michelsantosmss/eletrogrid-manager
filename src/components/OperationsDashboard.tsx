...
export function OperationsDashboard({ clients, equipment, orders, finance, quotes = [], demo }: Props) {
  ...
  const monthlyReceivable = monthLabels.map((_, month) => revenueEntries
    .filter((entry) => {
      const [year, entryMonth] = entry.dueDate.split('-').map(Number);
      return !entry.paid && year === currentYear && entryMonth === month + 1;
    })
    .reduce((sum, entry) => sum + netRevenue(entry), 0));
  const selectedRevenue = selectedMonth === null ? revenueTotal : monthlyRevenue[selectedMonth];
  const selectedReceivable = selectedMonth === null ? receivable : monthlyReceivable[selectedMonth];
  const selectedReceived = selectedRevenue - selectedReceivable;
  const selectedRevenueStatus = selectedRevenue === 0 ? 'Sem faturamento' : selectedReceivable === 0 ? 'Recebido' : selectedReceived > 0 ? 'Parcialmente recebido' : 'A receber';
