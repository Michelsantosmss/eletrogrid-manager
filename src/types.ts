...
export type Equipment = { id: string; clientId: string; equipmentName?: string; category: 'Eletrônico' | 'Energia solar' | 'Elétrico' | 'Instalação de ar-condicionado split'; brand: string; model: string; serial: string; color?: string; voltage?: string; accessories?: string; condition?: string; notes: string };
...
export type Quote = { id: string; code?: string; serviceOrderId: string; items: QuoteItem[]; discount: number; deadline: string; warranty: string; notes: string; approved: boolean; parts?: number; labor?: number };
