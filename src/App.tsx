...
const nextOrderId = (orders: ServiceOrder[]) => {
  const highest = orders.reduce(
    (current, order) =>
      Math.max(current, Number(order.id.match(/(\d+)$/)?.[1] ?? 0)),
    0,
  );
...
const equipmentQr = (orderId: string, equipmentId: string) => {
  ...
  return {
    value,
    url: `https://api.qrserver.com/v1/create-qr-code/?size=260x260&format=png&data=${encodeURIComponent(value)}`,
  };
};
...
export function resolveOrderRelations(
  ...
  const linkedEquipment = equipment.find(
    (entry) => entry.id === order.equipmentId,
  );
  const compatibleEquipment = directClient
    ? equipment.filter((entry) => entry.clientId === directClient.id)
    : equipment;
  const asset =
    linkedEquipment ??
    (compatibleEquipment.length === 1 ? compatibleEquipment[0] : undefined);
  const client =
    directClient ??
...
export function equipmentForClient(
  clientId: string,
  clients: Client[],
  equipment: Equipment[],
) {
...
export function orderFromQr(value: string, orders: ServiceOrder[]) {
  ...
  return orders.find(
    (order) =>
      order.qrCode?.value.toUpperCase() === normalized ||
...
function App() {
  ...
  const [equipment, setEquipment] = useRealtimeCollection<Equipment>(
    "equipment",
    equipmentSeed,
    cloudEnabled,
  );
  const [orders, setOrders] = useRealtimeCollection<ServiceOrder>(
    "serviceOrders",
    orderSeed,
    cloudEnabled,
  );
  const [quotes, setQuotes] = useRealtimeCollection<Quote>(
    "quotes",
    quoteSeed,
    cloudEnabled,
  );
  const [finance, setFinance] = useRealtimeCollection<FinanceEntry>(
    "finance",
    financeSeed,
    cloudEnabled,
  );
  ...
  const [editingOrderStatus, setEditingOrderStatus] =
    useState<ServiceStatus>("Recebido");
  ...
  useEffect(() => {
    ...
    setOrderForm((current) => {
      const clientId = clients.some((client) => client.id === current.clientId)
        ? current.clientId
        : clients[0].id;
      const available = equipmentForClient(clientId, clients, equipment);
      const equipmentId = available.some(
        (item) => item.id === current.equipmentId,
      )
        ? current.equipmentId
        : (available[0]?.id ?? "");
      return clientId === current.clientId &&
        ...
        ? current
        : { ...current, clientId, equipmentId };
    });
    setEquipmentForm((current) =>
      clients.some((client) => client.id === current.clientId)
        ? current
  ...
  async function handleAuth(event: FormEvent, mode: "login" | "register") {
    ...
    if (!firebaseEnabled) {
      setAuthError(
        "Configure as variáveis VITE_FIREBASE_* para usar a autenticação.",
      );
      return;
    }
    try {
      ...
      setAuthError(
        "Não foi possível autenticar. Confira o e-mail, senha e a configuração Firebase.",
      );
    }
  }
  ...
  async function handlePasswordReset() {
    ...
    if (!normalizedEmail) {
      setAuthError("Digite seu e-mail para redefinir a senha.");
      return;
    }
    ...
    if (!firebaseEnabled) {
      setAuthError(
        "A recuperação de senha não está disponível no modo demonstração.",
      );
      return;
    }
    try {
      ...
      setAuthMessage(
        "Enviamos o link de redefinição. Verifique sua caixa de entrada e a pasta de spam.",
      );
    } catch {
      setAuthError(
  ...
  async function persist<T extends { id: string }>(path: string, record: T) {
    if (demo) return;
    if (!cloudEnabled)
      throw new Error("Aguarde a conexão com a nuvem e tente novamente.");
  ...
  const cloudError = () =>
    window.alert(
      "Não foi possível salvar na nuvem. Verifique a internet, aguarde a sincronização e tente novamente.",
    );

  async function submitClient(event: FormEvent) {
    event.preventDefault();
    const record: Client = {
      id: editingClient ?? makeId("cli"),
      ...clientForm,
    };
    try {
  ...
  async function submitEquipment(event: FormEvent) {
    event.preventDefault();
    const clientId = clients.some(
      (client) => client.id === equipmentForm.clientId,
    )
      ? equipmentForm.clientId
      : (clients[0]?.id ?? "");
    if (!clientId) return;
    const record: Equipment = {
      id: editingEquipment ?? makeId("eq"),
      ...equipmentForm,
      clientId,
    };
    try {
  ...
  async function submitOrder(event: FormEvent) {
    event.preventDefault();
    const selectedEquipment = equipmentForClient(
      orderForm.clientId,
      clients,
      equipment,
    ).find((item) => item.id === orderForm.equipmentId);
    if (!selectedEquipment) return;
    if (editingOrderId) {
      const original = orders.find((order) => order.id === editingOrderId);
      if (!original) return;
      const qrCode =
        original.equipmentId === selectedEquipment.id && original.qrCode
          ? original.qrCode
          : equipmentQr(original.id, selectedEquipment.id);
      const documents = [
        ...(original.documents ?? []).filter(
          (document) => document.name !== "Etiqueta QR do equipamento",
        ),
        { name: "Etiqueta QR do equipamento", url: qrCode.url },
      ];
      const record: ServiceOrder = {
        ...
        history: [
          ...original.history,
          {
            ...
            status: editingOrderStatus,
            note: "Dados principais e status da OS atualizados.",
          },
        ],
      };
      try {
      ...
      setEditingOrderStatus("Recebido");
      setOrderForm({
        clientId: clients[0]?.id ?? "",
        equipmentId:
          equipmentForClient(clients[0]?.id ?? "", clients, equipment)[0]?.id ??
          "",
        problem: "",
        diagnosis: "",
      });
      ...
      history: [
        {
          at: new Date().toLocaleString("pt-BR"),
          status: "Recebido",
          note: `Entrada registrada. Etiqueta QR criada: ${qrCode.value}.`,
        },
      ],
      documents: [{ name: "Etiqueta QR do equipamento", url: qrCode.url }],
    };
    try {
      await persist("serviceOrders", record);
    } catch {
  ...
  async function attachFile(order: ServiceOrder, file: File) {
    try {
      const url =
        firebaseEnabled && !demo
          ? await uploadServiceFile(order.id, file)
          : URL.createObjectURL(file);
      await updateOrder(
        order,
        { documents: [...(order.documents ?? []), { name: file.name, url }] },
        `Anexo incluído: ${file.name}.`,
      );
    } catch (error) {
      const detail = error instanceof Error ? ` ${error.message}` : "";
      window.alert(
        `Não foi possível enviar o anexo. Verifique o acesso ao Supabase Storage e tente novamente.${detail}`,
      );
    }
  }
  async function createQuote(orderId: string) {
    const existing = quotes.find(
      (quote) => quote.serviceOrderId === orderId && !quote.approved,
    );
    setQuoteDraft(
      existing
        ? {
            ...existing,
            items: quoteItems(existing),
            deadline: existing.deadline || "5 dias úteis",
            warranty: existing.warranty || "90 dias sobre o serviço executado",
        ...
        : {
            ...
            warranty: "90 dias sobre o serviço executado",
            notes: "",
            approved: false,
          },
    );
  ...
  async function saveQuote(event: FormEvent) {
    ...
    setQuotes((items) =>
      exists
        ? items.map((quote) =>
            quote.id === quoteDraft.id ? quoteDraft : quote,
          )
        : [quoteDraft, ...items],
    );
    ...
    if (linkedOrder)
      await updateOrder(
        linkedOrder,
        { serviceValue: quoteTotal(quoteDraft) },
  ...
  async function approveQuote(quote: Quote) {
    ...
    try {
      await Promise.all([
        persist("quotes", approved),
        persist("finance", entry),
      ]);
    } catch {
      cloudError();
      return;
    ...
    setQuotes((items) =>
      items.map((item) => (item.id === quote.id ? approved : item)),
    );
    setFinance((items) => [entry, ...items]);
  }
  ...
  function cancelOrderEdit() {
    ...
    setOrderForm({
      clientId,
      equipmentId:
        equipmentForClient(clientId, clients, equipment)[0]?.id ?? "",
      problem: "",
      diagnosis: "",
    });
  }
  async function deleteOrder(order: ServiceOrder) {
    if (
      !window.confirm(
        `Excluir ${order.id.toUpperCase()}? A OS e seus orçamentos vinculados serão removidos. Os lançamentos financeiros serão preservados.`,
      )
    )
      return;
    const linkedQuotes = quotes.filter(
      (quote) => quote.serviceOrderId === order.id,
    );
    try {
      await Promise.all([
        destroy("serviceOrders", order.id),
        ...linkedQuotes.map((quote) => destroy("quotes", quote.id)),
      ]);
    } catch {
      cloudError();
      return;
    ...
    setQuotes((items) =>
      items.filter((quote) => quote.serviceOrderId !== order.id),
    );
    if (editingOrderId === order.id) cancelOrderEdit();
  }
  async function updateFinanceStatus(entry: FinanceEntry) {
    const updated = { ...entry, paid: !entry.paid };
    setFinance((items) =>
      items.map((item) => (item.id === entry.id ? updated : item)),
    );
    try {
      await persist("finance", updated);
    } catch {
      setFinance((items) =>
  ...
  function openOrderFromQr(value: string) {
    ...
    if (!order) {
      window.alert(
        "Nenhuma ordem de serviço foi encontrada para este QR Code.",
      );
      return;
    }
    setSearch(order.id);
  ...
  if (!userReady)
    return <main className="login-screen">Carregando sessão…</main>;
  if (!isAuthenticated)
    return (
      <main className="login-screen">
        ...
        >
          ...
          <p>Acesse a operação técnica com sua conta Firebase.</p>
          <input
            aria-label="E-mail"
            value={email}
          ...
          {authMessage && (
            <strong className="form-success" role="status">
              {authMessage}
            </strong>
          )}
      ...
      <section className="content">
        <header className="hero">
          <div>
            ...
            <h1>Controle técnico em tempo real.</h1>
            <p>
              Clientes, equipamentos e ordens de serviço em uma única operação.
            </p>
          </div>
          <div className="hero-actions">
            {activeModule === "Ordens de serviço" && (
              <QrCodeScanner onRead={openOrderFromQr} />
            )}
        ...
        {activeModule === "Equipamentos" && (
          <EquipmentModule
            ...
            editing={Boolean(editingEquipment)}
          />
        )}
        {activeModule === "Ordens de serviço" && (
          <Orders
...
function Clients({
  ...
  items: Client[];
  form: Omit<Client, "id">;
  setForm: (value: Omit<Client, "id">) => void;
  ...
  return (
    <Section eyebrow="Cadastros" title="Clientes" actions={<UsersRound />}>
      <form className="crud-form" onSubmit={onSubmit}>
        {(["name", "document", "phone", "email", "city"] as const).map(
          (key) => (
            <input
              key={key}
              value={form[key]}
        ...
        <button type="submit">
          {editing ? "Atualizar cliente" : "Salvar cliente"}
        </button>
      </form>
      <Cards>
        {items.map((item) => (
...
function EquipmentModule({
  ...
  clients: Client[];
  items: Equipment[];
  form: Omit<Equipment, "id">;
  setForm: (value: Omit<Equipment, "id">) => void;
  onSubmit: (e: FormEvent) => void;
  ...
  return (
    <Section eyebrow="Cadastros" title="Equipamentos" actions={<HardDrive />}>
      <p className="intake-help">
        Registre a identificação e as condições do aparelho no momento da
        entrada.
      </p>
      <form className="crud-form equipment-form" onSubmit={onSubmit}>
        <select
          aria-label="Cliente do equipamento"
          value={form.clientId}
            ...
            placeholder={
              {
                ...
                color: "Cor",
                voltage: "Tensão (ex.: 220 V)",
                accessories: "Acessórios recebidos",
                condition: "Estado na entrada",
                notes: "Observações de entrada",
              }[key]
            }
            required={
              key === "brand" ||
        ...
        <button type="submit" disabled={!clients.length}>
          {editing ? "Atualizar equipamento" : "Salvar equipamento"}
        </button>
      </form>
      <Cards>
        {items.map((item) => (
          <article className="record-card" key={item.id}>
            ...
            <span>
              {item.category} · Série/IMEI: {item.serial}
            </span>
            <span>
              Entrada: {item.condition ?? "Não informada"}
              {item.color ? ` · ${item.color}` : ""}
            </span>
            <span>Acessórios: {item.accessories || "Não informados"}</span>
...
function Orders({
  ...
  editingOrderStatus: ServiceStatus;
  equipment: Equipment[];
  quotes: Quote[];
  items: ServiceOrder[];
  form: {
  ...
  return (
    <Section eyebrow="Operação" title="Ordens de serviço" actions={<Wrench />}>
      ...
      {editingOrderId && (
        <div className="editing-banner">
          <strong>Editando {editingOrderId.toUpperCase()}</strong>
      ...
      <form className="crud-form" onSubmit={onSubmit}>
        ...
        <select
          aria-label="Equipamento da nova OS"
        ...
        {!availableEquipment.length && (
          <span className="form-error">
            Cadastre ou vincule um equipamento a este cliente.
          </span>
        )}
        <input
          value={form.problem}
        ...
        {editingOrderId && (
          <select
            aria-label="Status da OS em edição"
            value={editingOrderStatus}
            onChange={(event) =>
              setEditingOrderStatus(event.target.value as ServiceStatus)
            }
          >
            {statuses.map((status) => (
        ...
        <button type="submit" disabled={!form.clientId || !form.equipmentId}>
          {editingOrderId ? (
            <>
              <Pencil size={16} />
              Atualizar OS
      ...
      <div className="order-list">
        {items.map((item) => {
          const { client, equipment: asset } = resolveOrderRelations(
            item,
            clients,
            equipment,
          );
          const linkedQuote = quotes.find(
            (quote) => quote.serviceOrderId === item.id,
          );
          const serviceValue =
          ...
          return (
            <article className="record-card order-card" key={item.id}>
              <div className="order-summary-line">
                <div>
                  <strong>
                    {item.id.toUpperCase()} · {item.status}
                  </strong>
                  <span>
              ...
              <div className="service-details">
                <textarea
                  aria-label={`Serviços executados ${item.id}`}
                  value={item.servicePerformed ?? ""}
                  onChange={(e) =>
                    onUpdate(
                      item,
                      { servicePerformed: e.target.value },
                      "Serviços executados atualizados.",
                    )
                  }
                  placeholder="Serviços executados"
                />
                <input
                  aria-label={`Garantia ${item.id}`}
                  value={item.warranty ?? ""}
                  onChange={(e) =>
                    onUpdate(
                      item,
                ...
                <label className="service-value-field">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      onUpdate(
                        item,
                        { serviceValue: Number(e.target.value) || 0 },
                        "Valor do serviço alterado manualmente.",
                      )
                    }
                    required
                  />
                ...
                <textarea
                  aria-label={`Observações técnicas ${item.id}`}
              ...
              <div className="card-actions">
                <button
                  aria-label={`Editar OS ${item.id.toUpperCase()}`}
                  className="ghost-button"
                  onClick={() => onEdit(item)}
                  type="button"
                >
                ...
                <button
                  aria-label={`Excluir OS ${item.id.toUpperCase()}`}
                  className="danger-button"
                ...
                <OrderDocumentButton
                  ...
                  order={item}
                />
                <button onClick={() => onQuote(item.id)} type="button">
                  <FileText size={16} />
                  Criar orçamento
                </button>
                <ServiceNoteButton
                ...
                <label className="file-button">
                  Anexar arquivo
                  <input
                    aria-label={`Anexar arquivo ${item.id}`}
                    type="file"
                    onChange={(e) => {
              ...
              <details>
                <summary>Histórico completo</summary>
...
function Quotes({
  ...
  draft: Quote | null;
  items: Quote[];
  setDraft: (quote: Quote | null) => void;
        ...
        <form className="quote-form" onSubmit={onSave}>
          ...
          {draft.items.map((item) => (
            <div className="quote-item-row" key={item.id}>
              <input
                ...
                placeholder="Descrição do serviço, peça ou material"
                required
              />
              <select
          ...
          <div className="quote-conditions">
            <input
              ...
              onChange={(e) =>
                setDraft({ ...draft, discount: Number(e.target.value) || 0 })
              }
              placeholder="Desconto"
            />
            <input
              aria-label="Prazo de execução"
              value={draft.deadline}
              onChange={(e) => setDraft({ ...draft, deadline: e.target.value })}
              placeholder="Prazo de execução"
            />
            <input
              aria-label="Garantia do orçamento"
              value={draft.warranty}
              onChange={(e) => setDraft({ ...draft, warranty: e.target.value })}
              placeholder="Garantia"
            />
            <textarea
              aria-label="Condições e observações"
              value={draft.notes}
              onChange={(e) => setDraft({ ...draft, notes: e.target.value })}
              placeholder="Condições e observações"
            />
          </div>
          <div className="quote-summary">
            <span>Subtotal: {money.format(quoteSubtotal(draft))}</span>
            <span>Desconto: {money.format(draft.discount)}</span>
            <strong className="quote-total">
              TOTAL: {money.format(quoteTotal(draft))}
            </strong>
          </div>
            ...
            <article className="record-card" key={item.id}>
              ...
              <div className="card-actions">
                ...
                <button
                  ...
                  type="button"
                >
                  {item.approved ? "Aprovado" : "Aprovar e lançar"}
                </button>
              </div>
            </article>
          );
      ...
      <div className="metrics-grid">
        ...
        <article className="metric-card">
          ...
          <strong>
            {money.format(
              items
                .filter((item) => item.type === "Pagar" && !item.paid)
                .reduce((sum, item) => sum + item.amount, 0),
            )}
          </strong>
        </article>
        <article className="metric-card">
          <small>Fluxo</small>
          <strong>{money.format(cashFlow)}</strong>
        </article>
      </div>
      <Cards>
        {items.map((item) => (
...
function Cards({ children }: { children: React.ReactNode }) {
  return <div className="records-grid">{children}</div>;
}
export default App;
