...
function App() {
  ...
  const [clients, setClients] = useRealtimeCollection<Client>('clients', clientsSeed, cloudEnabled);
  const [equipment, setEquipment] = useRealtimeCollection<Equipment>('equipment', equipmentSeed, cloudEnabled);
  const [orders, setOrders] = useRealtimeCollection<ServiceOrder>('serviceOrders', orderSeed, cloudEnabled);
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {activeModule === 'Financeiro' && <Finance cashFlow={cashFlow} items={finance} onTogglePaid={updateFinanceStatus}/>}
    </section>
  </main>;
}

const match = (term: string, ...values: string[]) => values.join(' ').toLocaleLowerCase().includes(term.toLocaleLowerCase());
function Dashboard({ clients, equipment, orders, finance, demo }: { clients: Client[]; equipment: Equipment[]; orders: ServiceOrder[]; finance: FinanceEntry[]; demo: boolean }) { return <OperationsDashboard clients={clients} equipment={equipment} orders={orders} finance={finance} demo={demo}/>; }
...
function Finance({ items, cashFlow, onTogglePaid }: { items: FinanceEntry[]; cashFlow: number; onTogglePaid: (item: FinanceEntry) => void }) { const received = items.filter((item) => item.type === 'Receber' && item.paid).reduce((sum, item) => sum + item.amount, 0); const receivable = items.filter((item) => item.type === 'Receber' && !item.paid).reduce((sum, item) => sum + item.amount, 0); return <Section eyebrow="Financeiro" title="Contas e fluxo de caixa" actions={<Activity/>}><div className="metrics-grid"><article className="metric-card"><small>A receber</small><strong>{money.format(receivable)}</strong></article><article className="metric-card"><small>Recebido</small><strong>{money.format(received)}</strong></article><article className="metric-card"><small>Pagar</small><strong>{money.format(items.filter((item) => item.type === 'Pagar' && !item.paid).reduce((sum, item) => sum + item.amount, 0))}</strong></article><article className="metric-card"><small>Fluxo</small><strong>{money.format(cashFlow)}</strong></article></div><Cards>{items.map((item) => <article className="record-card" key={item.id}><strong>{item.type}</strong><span>{item.description}</span><span>{item.dueDate} · {item.paid ? 'Pago' : 'Aberto'}</span><strong>{money.format(item.amount)}</strong><div className="card-actions"><button className="ghost-button" onClick={() => onTogglePaid(item)} type="button">{item.paid ? 'Marcar como aberto' : item.type === 'Receber' ? 'Marcar como recebido' : 'Marcar como pago'}</button></div></article>)}</Cards></Section>; }
