...
function Dashboard({ clients, equipment, openOrders, cashFlow, live }: { clients: number; equipment: number; openOrders: number; cashFlow: number; live: boolean }) { const cards = [['Clientes ativos', clients, UsersRound], ['Equipamentos', equipment, HardDrive], ['OS abertas', openOrders, Wrench], ['Fluxo previsto', money.format(cashFlow), BanknoteArrowUp]] as const; return <section className="metrics-grid">{cards.map(([label, value, Icon]) => <article className="metric-card" key={label}><span><Icon size={22}/></span><small>{label}</small><strong>{value}</strong><em>{live ? 'Sincronizado agora' : 'Modo demonstração'}</em></article>)}</section>; }
