...
import { quoteItems, quoteTotal } from './services/quoteDocument';
import './styles.css';
...
function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="content">
      ...
      {activeModule === 'Orçamentos' && <Quotes draft={quoteDraft} items={quotes} onApprove={approveQuote} onCancel={() => setQuoteDraft(null)} onSave={saveQuote} setDraft={setQuoteDraft}/>}
...
function Orders({ clients, equipment, items, form, setForm, onSubmit, onStatus, onUpdate, onAttach, onQuote, statusFilter, setStatusFilter }: { clients: Client[]; equipment: Equipment[]; items: ServiceOrder[]; form: { clientId: string; equipmentId: string; problem: string; diagnosis: string }; setForm: (value: { clientId: string; equipmentId: string; problem: string; diagnosis: string }) => void; onSubmit: (e: FormEvent) => void; onStatus: (order: ServiceOrder, status: ServiceStatus) => void; onUpdate: (order: ServiceOrder, changes: Partial<ServiceOrder>, note?: string) => void; onAttach: (order: ServiceOrder, file: File) => void; onQuote: (id: string) => void; statusFilter: 'Todas' | ServiceStatus; setStatusFilter: (value: 'Todas' | ServiceStatus) => void }) {
  return <Section eyebrow="Operação" title="Ordens de serviço" actions={<Wrench/>}>
    ...
    <form className="crud-form" onSubmit={onSubmit}><select aria-label="Cliente da nova OS" value={form.clientId} onChange={(e) => setForm({ ...form, clientId: e.target.value })}>{clients.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select aria-label="Equipamento da nova OS" value={form.equipmentId} onChange={(e) => setForm({ ...form, equipmentId: e.target.value })}>{equipment.filter((item) => item.clientId === form.clientId).map((item) => <option key={item.id} value={item.id}>{item.brand} {item.model}</option>)}</select><input value={form.problem} onChange={(e) => setForm({ ...form, problem: e.target.value })} placeholder="Problema relatado" required/><input value={form.diagnosis} onChange={(e) => setForm({ ...form, diagnosis: e.target.value })} placeholder="Diagnóstico inicial"/><button type="submit"><Plus size={16}/>Gerar OS automaticamente</button></form>
    <div className="order-list">{items.map((item) => { const client = clients.find((entry) => entry.id === item.clientId); const asset = equipment.find((entry) => entry.id === item.equipmentId); return <article className="record-card order-card" key={item.id}><div className="order-summary-line"><div><strong>{item.id.toUpperCase()} · {item.status}</strong><span>Entrada: {item.intakeDate}{item.exitDate ? ` · Saída: ${item.exitDate}` : ''}</span><p>{item.problem}</p></div><select aria-label={`Status ${item.id}`} value={item.status} onChange={(e) => onStatus(item, e.target.value as ServiceStatus)}>{statuses.map((status) => <option key={status}>{status}</option>)}</select></div><div className="service-details"><textarea aria-label={`Serviços executados ${item.id}`} value={item.servicePerformed ?? ''} onChange={(e) => onUpdate(item, { servicePerformed: e.target.value }, 'Serviços executados atualizados.')} placeholder="Serviços executados"/><input aria-label={`Garantia ${item.id}`} value={item.warranty ?? ''} onChange={(e) => onUpdate(item, { warranty: e.target.value }, 'Garantia atualizada.')} placeholder="Garantia do serviço"/><textarea aria-label={`Observações técnicas ${item.id}`} value={item.technicianNotes ?? ''} onChange={(e) => onUpdate(item, { technicianNotes: e.target.value }, 'Observações técnicas atualizadas.')} placeholder="Observações técnicas"/></div><div className="card-actions"><OrderDocumentButton client={client} equipment={asset} order={item}/><button onClick={() => onQuote(item.id)} type="button"><FileText size={16}/>Criar orçamento</button><ServiceNoteButton client={client} equipment={asset} order={item}/><label className="file-button">Anexar arquivo<input aria-label={`Anexar arquivo ${item.id}`} type="file" onChange={(e) => { const file = e.target.files?.[0]; if (file) onAttach(item, file); }}/></label></div>{item.documents?.length ? <div className="document-list">{item.documents.map((doc) => <a href={doc.url} key={`${item.id}-${doc.name}`} rel="noreferrer" target="_blank">{doc.name}</a>)}</div> : null}<details><summary>Histórico completo</summary>{item.history.map((entry, index) => <p key={`${entry.at}-${index}`}>{entry.at} · {entry.status} · {entry.note}</p>)}</details></article>; })}</div>
  </Section>;
}

function Quotes({ draft, items, setDraft, onSave, onCancel, onApprove }: { draft: Quote | null; items: Quote[]; setDraft: (quote: Quote | null) => void; onSave: (event: FormEvent) => void; onCancel: () => void; onApprove: (quote: Quote) => void }) {
