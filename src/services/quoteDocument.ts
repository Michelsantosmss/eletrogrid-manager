...
export async function printQuote(quote: Quote) {
  const { pdf, y: startY } = await createPdf('ORÇAMENTO', `${quote.id.toUpperCase()} - ${quote.serviceOrderId.toUpperCase()}`);
  let y = addSection(pdf, startY, 'Serviços, peças e materiais', quoteItems(quote).map((item) => `${item.quantity} x ${item.description || item.kind} - ${money.format(item.unitPrice)} = ${money.format(item.quantity * item.unitPrice)}`).join('\n'));
